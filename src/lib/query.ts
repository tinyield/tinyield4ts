...
export class Query<T> extends Advancer<T> {
    ...
    public constructor(source: Advancer<T>, characteristics?: Characteristics) {
        super();
        this.adv = source;
        if (characteristics !== undefined) {
            this.characteristics = characteristics;
        } else {
            this.characteristics = DEFAULT_CHARACTERISTICS;
        }
    }
    ...
    public static of<T>(source: T[]): Query<T> {
        return new Query<T>(new AdvancerArray(source), DEFAULT_CHARACTERISTICS);
    }
    ...
    public static iterate<T>(seed: T, operation: (elem: T) => T): Query<T> {
        return new Query<T>(new AdvancerIterator(seed, operation), DEFAULT_CHARACTERISTICS);
    }
    ...
    static generate<T>(supplier: () => T): Query<T> {
        return new Query<T>(new AdvancerGenerate(supplier), DEFAULT_CHARACTERISTICS);
    }
    ...
    sorted(comparator: (a: T, b: T) => number): Query<T> {
        return new Query<T>(new AdvancerArray(this.toArray().sort(comparator)), this.characteristics);
    }
    ...
    filter(predicate: (elem: T) => boolean): Query<T> {
        return new Query<T>(new AdvancerFilter(this, predicate), this.characteristics);
    }
    ...
    map<R>(mapper: (elem: T) => R): Query<R> {
        return new Query(new AdvancerMap(this.adv, mapper), this.characteristics);
    }
    ...
    skip(n: number): Query<T> {
        return new Query<T>(new AdvancerSkip(this.adv, n), this.characteristics);
    }
    ...
    take(n: number): Query<T> {
        return new Query<T>(new AdvancerTake(this, n), this.characteristics);
    }
    ...
    takeWhile(predicate: (elem: T) => boolean): Query<T> {
        return new Query<T>(new AdvancerTakeWhile(this, predicate), this.characteristics);
    }
    ...
    first(): T {
        let first: T;
        if (this.characteristics.hasAdvancer) {
    ...
    distinctBy(selector: (elem: T) => any): Query<T> {
        return new Query<T>(new AdvancerDistinct(this.adv, selector), this.characteristics);
    }
    ...
    then<U>(next: (source: Query<T>) => Traverser<U>): Query<U> {
        return new Query<U>(new AdvancerThen(this, next), new Characteristics(false));
    }
    ...
    zip<U, R>(other: Query<U>, zipper: (elem1: T, elem2: U) => R): Query<R> {
        return new Query<R>(new AdvancerZip(this, other, zipper), this.characteristics);
    }
    ...
    flatMap<R>(mapper: (elem: T) => Query<R>): Query<R> {
        return new Query<R>(new AdvancerFlatmap(this, mapper), this.characteristics);
    }
    ...
    peek(action: (elem: T) => void): Query<T> {
        return new Query<T>(new AdvancerPeek(this.adv, action), this.characteristics);
    }
    ...
    concat(other: Query<T>): Query<T> {
        return new Query<T>(new AdvancerConcat(this.adv, other.adv), this.characteristics);
    }
    ...
    dropWhile(predicate: (elem: T) => boolean): Query<T> {
        return new Query<T>(new AdvancerDropWhile(this.adv, predicate), this.characteristics);
    }
